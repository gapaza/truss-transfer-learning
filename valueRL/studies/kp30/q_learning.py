...
class QLearning:
    ...
    def run(self):
        ...
        for x in range(self.epochs):
            ...
            for x in range(update_batch_iterations):
                epoch_info = self.train_q_network(use_pop=False)
            if epoch_info:
    ...
    def train_q_network(self, use_pop=False):
        ...
        # sample from most recent 100 epochs
        # sample_space = 200 * global_mini_batch_size
        # if len(self.replay_buffer) < sample_space:
        #     update_batch = random.sample(self.replay_buffer, update_batch_size)
        # else:
        #     update_batch = random.sample(self.replay_buffer[:sample_space], update_batch_size)
        # update_batch = random.sample(self.replay_buffer, update_batch_size)
        # update_batch += random.sample(pop_memories, 5)
        # print(update_batch[0])
        ...
        loss = self.train_value_network(observation_tensor, cross_obs_tensor, actions_tensor, target_values)
        loss = loss.numpy()
    ...
    def train_value_network(self, observation, cross_obs_tensor, actions, target_values):
    ...
    def sample_value_network(self, observation, cross_obs_tensor):
        ...
        #     temperature = 0.7
        #     q_values_temp = q_values_t / temperature
        #     q_values_log_probs = tf.nn.log_softmax(q_values_temp, axis=-1)
        #     samples = tf.random.categorical(q_values_log_probs, 1)  # shape (batch, 1)
        #     actions = []
        #     actions_values = []
        #     for idx, sample in enumerate(samples):
        #         action = sample.numpy()[0]
        #         actions.append(action)
        #         actions_values.append(q_values[idx][action])
