...
@keras.saving.register_keras_serializable(package="AlibiBias", name="AlibiBias")
class AlibiBias(tf.keras.layers.Layer):
    """A layer that adds the alibi bias to attention scores.
        ...
        """
    def __init__(self, alibi_bias_max=8, **kwargs):
        super().__init__(**kwargs)
        self.supports_masking = True
        self.alibi_bias_max = alibi_bias_max

    def call(self, attention_scores):
        shape = tf.shape(attention_scores)
        if len(shape) != 4:
            raise ValueError(
        ...
        return tf.add(attention_scores, alibi_bias)

    
    def _get_alibi_bias(self, num_heads, key_length):
        slopes = tf.convert_to_tensor(
            self._get_slopes(num_heads), dtype=self.compute_dtype
        )
        slopes = tf.expand_dims(slopes, 1)

        seq_range = tf.expand_dims(tf.arange(1 - key_length, 1), 0)
        seq_range = tf.cast(seq_range, dtype=self.compute_dtype)

        alibi_bias = tf.multiply(slopes, seq_range)
        alibi_bias = tf.expand_dims(alibi_bias, 1)

        # return shape is `(1, num_heads, 1, key_length)`
        return tf.expand_dims(alibi_bias, 0)
