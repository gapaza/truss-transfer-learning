...
task_epochs = 10000
clip_ratio = 0.2
target_kl = 0.0005  # was 0.0001 then 0.0005
entropy_coef = 0.00
use_actor_warmup = True


perf_term_weight = 1.0
constraint_pred_weight = 0.0
constraint_term_weight = 1.0  # was 0.2
def get_constraint_term_weight(epoch_num):
    # return constraint_term_weight
    if epoch_num < 0:
        return 0.0
    else:
        return constraint_term_weight
...
class PPO_Constrained_MultiTaskValidation(AbstractTask):
    ...
    def __init__(
        ...
        self.objective_weights = list(np.linspace(0.05, 0.95, num_keys))  # was 0.95, then 0.3
    ...
    def build(self):
        ...
        self.actor_learning_rate = 0.00001  # 0.00001
    ...
    def fast_mini_batch(self):
        ...
        all_logprobs = [[] for _ in range(self.mini_batch_size)]  # Logprobs for only selection action
        ...
        for t in range(self.steps_per_design):
            ...
            # print(action_log_prob.shape)  # (9,)
            # print(all_action_log_probs.shape)  # (9, 2)
        ...
        # print(logprob_tensor.shape)  # (9, 280)
        # print(all_logprobs_tensor.shape)  # (9, 280, 2)

        stiffness_ratio_tensor = tf.convert_to_tensor(all_stiffness_ratio_deltas, dtype=tf.float32)
        stiffness_ratio_tensor = tf.expand_dims(stiffness_ratio_tensor, axis=-1)
        ...
        curr_time = time.time()
        policy_update_itr = 0
        kl, entr, policy_loss, actor_loss, constraint_loss = 0, 0, 0, 0, 0
        if update_actor is True:
            for i in range(self.train_actor_iterations):
                policy_update_itr += 1
                kl, entr, policy_loss, actor_loss, constraint_loss, true_kl = self.train_actor(
                    observation_tensor,
                    action_tensor,
                    logprob_tensor,
                    advantage_tensor,
                    cross_obs_tensor,
                    stiffness_ratio_tensor,
                    all_logprobs_tensor,
                    all_probs_tensor
                )
                self.actor_steps += 1
                if true_kl > 1.5 * self.target_kl:
                    # Early Stopping
                    break
            kl = kl.numpy()
            entr = entr.numpy()
    ...
    def calc_reward(self, bitstr, weight, task, run_val=True):
        ...
        # Add performance penalty if vol_frac is 1 and v_stiffness is 0
        # penalty = 0.0
        # if vol_frac == 1.0 and v_stiffness == 0.0:
        #     penalty = 1.0
        # performance_term -= penalty
        ...
        if in_stiffness_window is False:
            stiffness_ratio_term = deepcopy(stiffness_ratio_delta)
            if stiffness_ratio_term > 10.0:
        ...
        return reward, design, constraint_term, stiff_ratio
    ...
    def _sample_actor(self, observation_input, cross_input, inf_idx):
        ...
        all_token_log_probs = tf.math.log(all_token_probs)
    ...
    def train_actor(
        ...
        with tf.GradientTape() as tape:
            pred_probs, pred_constraint = self.c_actor.call_constraint([observation_buffer, parent_buffer], training=use_actor_train_call)  # shape: (batch, seq_len, 2)
            ...
            # loss = loss + (constraint_loss * constraint_pred_weight)
        ...
        true_kl = tf.reduce_mean(true_kl)  # shape (1,)


        return kl, entr, policy_loss, loss, constraint_loss, true_kl
    ...
    def plot_ppo(self):
        ...
        # iterate over unique designs in reverse order
        unique_design_vals_reversed = self.unique_designs_vals[::-1]
        unique_design_feasible_reversed = self.unique_designs_feasible[::-1]

        # Prepare non-feasible points
        non_feasible_x = np.array([val[0] * -1.0 for idx, val in enumerate(unique_design_vals_reversed) if not unique_design_feasible_reversed[idx]])
        non_feasible_y = np.array([val[1] for idx, val in enumerate(unique_design_vals_reversed) if not unique_design_feasible_reversed[idx]])
        # Prepare feasible points
        feasible_x = np.array([val[0] * -1.0 for idx, val in enumerate(self.unique_designs_vals) if self.unique_designs_feasible[idx]])
        feasible_y = np.array([val[1] for idx, val in enumerate(self.unique_designs_vals) if self.unique_designs_feasible[idx]])
        # Plot all points with just two scatter plot calls
        plt.scatter(non_feasible_x, non_feasible_y, color='grey', alpha=0.3)
        plt.scatter(feasible_x, feasible_y, color='blue')
        # for idx, obj_vals in enumerate(unique_design_vals_reversed):
        #     if unique_design_feasible_reversed[idx] is False:
