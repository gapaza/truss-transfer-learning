...
class PPO_Constrained_Task(AbstractTask):
    ...
    def __init__(
        ...
        super(PPO_Constrained_Task, self).__init__(run_num, barrier, problem, epochs, actor_load_path, critic_load_path)
        self.debug = debug
        self.c_type = c_type
        ...
        for t in range(self.steps_per_design):
            ...
            if len(designs[0]) == self.steps_per_design:
                ...
                for idx, design in enumerate(designs):
                    ...
                    # Evaluate design
                    reward, design_obj, constraint_term = self.calc_reward(
                        design_bitstr,
                    ...
                    all_constraint_returns.append(constraint_term)
            else:
                done = False
                reward = 0.0
    ...
    def plot_ppo(self):
        ...
        # Constraint Term History
        plt.subplot(gs[3, 1])
        plt.plot(epochs, self.constraint_term_history)
        plt.xlabel('Epoch')
        plt.ylabel('Constraint Term Weight')
        plt.title('Constraint Term Weight History')
