...
class PPO_Constrained_MultiTaskValidation2(AbstractTask):
    ...
    def __init__(
        ...
        super(PPO_Constrained_MultiTaskValidation2, self).__init__(run_num, barrier, problem, epochs, actor_load_path, critic_load_path)
        self.debug = debug
        self.c_type = c_type
        ...
        self.unique_designs_weight = []
        self.unique_designs_constraint = []
        ...
        # Constraint Weights
        num_constraint_keys = 2
        self.constraint_weights = [x for x in range(num_constraint_keys)]
        self.constraint_weights_vals = list(np.linspace(0.00, 0.2, num_constraint_keys))  # was 0.00, 0.05
        self.constraint_weights_norm = list(np.linspace(0.00, 1.0, num_constraint_keys))
        print(self.constraint_weights_vals)
        print(self.constraint_weights_norm)
    ...
    def get_cross_obs(self):
        ...
        # Constraint Weight Sampling
        constraint_weight_samples = random.sample(self.constraint_weights, num_constraint_samples)
        ...
        for task in task_samples:
            ...
            for weight in weight_samples:
                for constraint_weight in constraint_weight_samples:
                    sample_vars = [weight, task_cond_vars[1], task_cond_vars[2], task_cond_vars[3], constraint_weight]
                    ...
                    constraint_samples_all.append(constraint_weight)
        ...
        for t in range(self.steps_per_design):
            ...
            if len(designs[0]) == self.steps_per_design:
                ...
                for idx, design in enumerate(designs):
                    ...
                    constraint = constraint_samples_all[idx]
                    if constraint == self.constraint_weights[-1]:
    ...
    def calc_reward(self, bitstr, weight, task, constraint_w, run_val=True):
        ...
        constraint_term = constraint_term * constraint_w
        ...
        if bitstr not in self.unique_designs:
            ...
            self.unique_designs_weight.append(weight)
            self.unique_designs_constraint.append(constraint_w)
            self.nfe += 1

        return reward, design, constraint_term, stiffness_ratio_delta
...
if __name__ == '__main__':
    ...
    alg = PPO_Constrained_MultiTaskValidation2(
        run_num=run_dir,
        problem=problem,
        epochs=task_epochs,
