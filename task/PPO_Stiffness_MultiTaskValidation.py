...
class PPO_Stiffness_MultiTask(AbstractTask):
    ...
    def __init__(
        ...
        self.val_task = target_tasks[val_task_num]
        self.run_tasks = [self.val_task]
        self.problem.target_stiffness_ratio = self.val_task[0]
        self.problem.feasible_stiffness_delta = str_delta
        print(self.problem.target_stiffness_ratio, self.problem.feasible_stiffness_delta)
    ...
    def calc_reward(self, bitstr, weight, task, run_val=True):
        ...
        if stiffness_ratio_delta < str_delta:
...
if __name__ == '__main__':
    ...
    # actor_save_path = os.path.join(config.results_save_dir, 'run_' + str(run_dir), 'pretrained', 'actor_weights_1200')
    # critic_save_path = os.path.join(config.results_save_dir, 'run_' + str(run_dir), 'pretrained', 'critic_weights_1200')
