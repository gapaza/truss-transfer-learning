...
min_str, max_str, str_delta = 0.6, 1.4, 0.001
target_ratios, target_ratios_norm = get_str_targets(min_str, max_str, str_delta)
target_tasks = []
...
class PPO_Stiffness_MultiTask(AbstractTask):
    ...
    def fast_mini_batch(self):
        ...
        for t in range(self.steps_per_design):
            action_log_prob, action, all_action_probs = self.sample_actor(observation, cross_obs_tensor, cross_obs_mask_tensor)  # returns shape: (batch,) and (batch,)
    ...
    @tf.function(input_signature=[
        tf.TensorSpec(shape=(None, 31), dtype=tf.float32),
        tf.TensorSpec(shape=(None, 31, 1), dtype=tf.float32),
        tf.TensorSpec(shape=(None, num_conditioning_vars), dtype=tf.float32),
        tf.TensorSpec(shape=(None, num_conditioning_vars), dtype=tf.float32),
    ])
    def train_critic(
