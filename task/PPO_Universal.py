...
from inference.gen_universal_task import sample_tasks
...
class PPO_Universal(AbstractTask):
    ...
    def __init__(
        ...
        self.all_tasks, self.all_task_positions, self.all_task_categories, self.all_task_category_types = sample_tasks()
        self.curr_len = 0
    ...
    def get_cross_obs(self):
        ...
        task_category = random.sample(self.all_task_categories, 1)[0]
        task_samples = random.sample(self.all_task_category_types, num_representations)
        all_task_positions = self.all_task_positions[task_category]
        self.curr_len = len(all_task_positions[0])
        ...
        for task in task_samples:
            pos_vec = all_task_positions[task]
            for weight in weight_samples:
                ...
                task_categories_all.append(task_category)
        ...
        task_categories_all = [element for element in task_categories_all for _ in range(repeat_size)]

        cross_obs_tensor = tf.convert_to_tensor(cross_obs_vars, dtype=tf.float32)

        return cross_obs_tensor, weight_samples_all, task_samples_all, task_categories_all
    ...
    def fast_mini_batch(self):
        ...
        cross_obs_tensor, weight_samples_all, task_samples_all, task_categories_all = self.get_cross_obs()
        ...
        # -------------------------------------
        curr_time = time.time()
        print('--> DESIGNS WITH LENGTH:', self.curr_len)

        for t in range(self.curr_len):
            ...
            if len(designs[0]) == self.curr_len:
                ...
                for idx, design in enumerate(designs):
                    ...
                    converted_design = self.convert_design(design_bitstr, task_samples_all[idx], task_categories_all[idx])
    ...
    def convert_design(self, bitstr, task, task_category):
        bit_list = [int(x) for x in bitstr]
        task_connections = self.all_tasks[task_category][task]
        design_connections = []
...
if __name__ == '__main__':
    ...
    alg = PPO_Universal(
        ...
        run_val=True,
        val_itr=run_num,
        num_tasks=9,  # Tested with 9 tasks, try upping substantially
    )
    alg.run()
