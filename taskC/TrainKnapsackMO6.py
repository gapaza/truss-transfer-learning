...
from problem.knapsack.KnapsackDesignMO6 import KnapsackDesignMO6 as Design
import scipy.signal
from task.GA_KnapsackMO6_Task import GA_KnapsackMO6_Task
from modelC import get_multi_task_decoder as get_model
from taskC import sample_many_obj_weights
...
class TrainKnapsackMO6(AbstractTask):
    ...
    def __init__(
        ...
        super(TrainKnapsackMO6, self).__init__(run_num, barrier, problem, epochs, actor_load_path, critic_load_path)
        self.debug = debug
        self.c_type = c_type
        ...
        # HV
        self.ref_point = np.array(config.hv_ref_point_mo6)  # value, weight
        ...
        # Objective Weights
        self.objective_weights = sample_many_obj_weights(6, total=1)
    ...
    def init_comparison_data(self):
        print('--> RUNNING GA')
        task_runner = GA_KnapsackMO6_Task(
    ...
    def get_cross_obs(self):
        ...
        weight_samples = random.sample(self.objective_weights, num_weight_samples - 6)
        weight_samples.append([0.9, 0.02, 0.02, 0.02, 0.02])
        weight_samples.append([0.02, 0.9, 0.02, 0.02, 0.02])
        weight_samples.append([0.02, 0.02, 0.9, 0.02, 0.02])
        weight_samples.append([0.02, 0.02, 0.02, 0.9, 0.02])
        weight_samples.append([0.02, 0.02, 0.02, 0.02, 0.9])
        weight_samples.append([0.02, 0.02, 0.02, 0.02, 0.02])


        # 5 weight samples
        cross_obs_vars = []
        weight_samples_all = []
        weight_samples_2_all = []
        weight_samples_3_all = []
        weight_samples_4_all = []
        weight_samples_5_all = []

        task_samples_all = []
        for weight, weight_2, weight_3, weight_4, weight_5 in weight_samples:
            # sample_vars = [weight, task_cond_vars[1], task_cond_vars[2], task_cond_vars[3]]
            sample_vars = [weight, weight_2, weight_3, weight_4, weight_5]
            cross_obs_vars.append(sample_vars)
            weight_samples_all.append(weight)
            weight_samples_2_all.append(weight_2)
            weight_samples_3_all.append(weight_3)
            weight_samples_4_all.append(weight_4)
            weight_samples_5_all.append(weight_5)
        ...
        weight_samples_2_all = [element for element in weight_samples_2_all for _ in range(repeat_size)]
        weight_samples_3_all = [element for element in weight_samples_3_all for _ in range(repeat_size)]
        weight_samples_4_all = [element for element in weight_samples_4_all for _ in range(repeat_size)]
        weight_samples_5_all = [element for element in weight_samples_5_all for _ in range(repeat_size)]
        ...
        return cross_obs_tensor, weight_samples_all, task_samples_all, weight_samples_2_all, weight_samples_3_all, weight_samples_4_all, weight_samples_5_all
    ...
    def fast_mini_batch(self):
        ...
        cross_obs_tensor, weight_samples_all, task_samples_all, weight_samples_2_all, weight_samples_3_all, weight_samples_4_all, weight_samples_5_all = self.get_cross_obs()
        str_tracker = {}
            ...
            if len(designs[0]) == self.steps_per_design:
                ...
                for idx, design in enumerate(designs):
                    ...
                    reward, design_obj, constraint_return, stiff_ratio_delta, feasibility_term, connectivity_term, stiffness_ratio_delta = self.calc_reward(
                        ...
                        weight_samples_2_all[idx],
                        weight_samples_3_all[idx],
                        weight_samples_4_all[idx],
                        weight_samples_5_all[idx],
    ...
    def calc_reward(self, bitstr, weight, weight_2, weight_3, weight_4, weight_5, task, run_val=True):
        ...
        obj_value, obj_value_2, obj_value_3, obj_value_4, obj_value_5, obj_weight = self.problem.evaluate(bitstr)
        ...
        value_1_term = w1 * obj_value
        value_2_term = w2 * obj_value_2
        value_3_term = w3 * obj_value_3
        value_4_term = w4 * obj_value_4
        value_5_term = w5 * obj_value_5
        weight_term = w6 * (1.0 - obj_weight)
        performance_term = value_1_term + value_2_term + value_3_term + value_4_term + value_5_term + weight_term
        ...
        d_objectives = [obj_value * -1.0, obj_value_2 * -1.0, obj_value_3 * -1.0, obj_value_4 * -1.0, obj_value_5 * -1.0, obj_weight]
        design.set_objectives(d_objectives)
        design.evaluated = True
        if bitstr not in self.unique_designs:
            self.unique_designs.add(bitstr)
            self.unique_designs_vals.append(d_objectives)
            self.unique_designs_feasible.append(design.is_feasible)
            self.unique_designs_epoch.append(self.curr_epoch)
            self.nfe += 1

        return reward, design, 0, 0, 0, 0, 0
    ...
    def plot_ppo(self):
        ...
        for idx, obj_vals in enumerate(self.unique_designs_vals):
            if self.unique_designs_feasible[idx] is True:
                ...
                feasible_y.append(obj_vals[2])
            else:
                infeasible_x.append(obj_vals[0] * -1.0)
                infeasible_y.append(obj_vals[2])
        ...
        for idx, obj_vals in enumerate(self.unique_designs_vals):
            if self.unique_designs_feasible[idx] is True:
                feasible_x.append(obj_vals[1] * -1.0)
                feasible_y.append(obj_vals[2])
            else:
                infeasible_x.append(obj_vals[1] * -1.0)
                infeasible_y.append(obj_vals[2])
        plt.scatter(feasible_x, feasible_y, color='blue')
        plt.scatter(infeasible_x, infeasible_y, color='grey', alpha=0.3)
        plt.xlim(0, 1.1)
...
from problem.knapsack.ManyObjective6KP import ManyObjective6KP

if __name__ == '__main__':
    problem = ManyObjective6KP(
        n=config.num_vars,
        new_problem=False
    )
