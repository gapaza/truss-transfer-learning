...
from problem.knapsack.KnapsackDesignMO import KnapsackDesignMO as Design
import scipy.signal
from task.GA_KnapsackMO_Task import GA_KnapsackMO_Task
...
class TrainKnapsackMT(AbstractTask):
    ...
    def __init__(
        ...
        super(TrainKnapsackMT, self).__init__(run_num, barrier, problem, epochs, actor_load_path, critic_load_path)
        self.debug = debug
        self.c_type = c_type
        ...
        # HV
        self.ref_point = np.array(config.hv_ref_point_mo)  # value, weight
        ...
        # Objective Weights
        self.objective_weights = sample_many_obj_weights(3, total=1)
    ...
    def init_comparison_data(self):
        print('--> RUNNING GA')
        task_runner = GA_KnapsackMO_Task(
    ...
    def get_cross_obs(self):
        ...
        weight_samples = random.sample(self.objective_weights, num_weight_samples - 3)
        weight_samples.append([0.9, 0.05])
        weight_samples.append([0.05, 0.9])
        weight_samples.append([0.05, 0.05])
        ...
        return cross_obs_tensor, weight_samples_all, task_samples_all, weight_samples_2_all
    ...
    def fast_mini_batch(self):
        ...
        cross_obs_tensor, weight_samples_all, task_samples_all, weight_samples_2_all = self.get_cross_obs()
        str_tracker = {}
    ...
    def calc_reward(self, bitstr, weight, weight_2, task, run_val=True):
        ...
        obj_value, obj_value_2, obj_weight = self.problem.evaluate(bitstr)
        ...
        weight_term = w3 * (1.0 - obj_weight)
        performance_term = value_1_term + value_2_term + weight_term
        ...
        d_objectives = [obj_value * -1.0, obj_value_2 * -1.0, obj_weight]
...
from problem.knapsack.ManyObjectiveSimpleKP import ManyObjectiveSimpleKP

if __name__ == '__main__':
    problem = ManyObjectiveSimpleKP(
        n=config.num_vars,
        new_problem=False
    )
