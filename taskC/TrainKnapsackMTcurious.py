...
from modelC import get_multi_task_decoder_curious as get_model
...
class TrainKnapsackMTcurious(AbstractTask):
    ...
    def __init__(
        ...
        super(TrainKnapsackMTcurious, self).__init__(run_num, barrier, problem, epochs, actor_load_path, critic_load_path)
        self.debug = debug
        self.c_type = c_type
        ...
        self.curious_model = None
        self.curious_optimizer = None
    ...
    def build(self):
        ...
        self.train_curiousity_iterations = 40  # was 40
        self.beta_1 = 0.9
        ...
        if self.curious_optimizer is None:
            self.curious_optimizer = tf.keras.optimizers.legacy.Adam(learning_rate=self.actor_learning_rate)

        self.curious_loss_fn = tf.keras.losses.CategoricalCrossentropy(from_logits=False)

        

        self.c_actor, self.c_critic, self.curious_model = get_model(self.actor_load_path, self.critic_load_path)
    ...
    def fast_mini_batch(self):
        ...
        curiosity_observation = deepcopy(observation)
        curiosity_observation = tf.convert_to_tensor(curiosity_observation, dtype=tf.float32)
        curiosity_t = self.sample_curiosity(curiosity_observation, cross_obs_tensor)  # (batch, max_seq_len, 2)
        curiosity_probs = tf.convert_to_tensor(curiosity_t, dtype=tf.float32)
        actor_probs = tf.convert_to_tensor(all_probs_full, dtype=tf.float32)
        # print('Curiosity Shape:', curiosity_probs.shape, '| Actor Shape:', actor_probs.shape)
        # print('Curiosity:', curiosity_probs, '| Actor:', actor_probs)
        # c_loss = self.curious_loss_fn(actor_probs, curiosity_probs)
        # print('Curiosity Loss:', c_loss)

        # The loss of the curiosity model is the cross entropy loss between actor action probs and curiosity model action probs
        # The intrinsic reward for the actor is the same as the loss for the curiosity model
        ...
        curr_time = time.time()
        policy_update_itr = 0
        kl, entr, policy_loss, actor_loss, cur_loss = 0, 0, 0, 0, 0
        if update_actor is True:
            for i in range(self.train_actor_iterations):
                policy_update_itr += 1
                kl, entr, policy_loss, actor_loss, cur_loss = self.train_actor(
                    observation_tensor,
                    action_tensor,
                    logprob_tensor,
                    advantage_tensor,
        ...
        curiosity_loss = 0
        for i in range(self.train_curiousity_iterations):
            curiosity_loss = self.train_curiosity(
                curiosity_observation,
                cross_obs_tensor,
                actor_probs
            )
            self.critic_steps += 1
            curiosity_loss = curiosity_loss.numpy()
        ...
        epoch_info = {
            ...
            'curiosity_model_loss': curiosity_loss,
            # 'curiosity_actor_loss': cur_loss,
            'c_loss': value_loss,
            'p_loss': policy_loss,
    ...
    def sample_curiosity(self, observation, parent_obs):
        # observation_input = tf.convert_to_tensor(observation, dtype=tf.float32)
        return self._sample_curiosity(observation, parent_obs)

    @tf.function(input_signature=[
        tf.TensorSpec(shape=(None, None), dtype=tf.float32),
        tf.TensorSpec(shape=(None, None), dtype=tf.float32),
    ])
    def _sample_curiosity(self, observation_input, cross_input):
        pred_probs = self.curious_model([observation_input, cross_input], training=use_actor_train_call)  # shape (batch, seq_len, 2)
        return pred_probs
    ...
    def train_actor(
        ...
        with tf.GradientTape() as tape:
            ...
            # Curiosity Term
            curiosity_loss = self.curious_loss_fn(all_probs_buffer, curiosity_probs)  # Higher positive value means tricking the curiosity model
        ...
        return kl, entr, policy_loss, loss, curiosity_loss
    ...
    def train_curiosity(
        ...
        with tf.GradientTape() as tape:
            pred_probs = self.curious_model([observation_buffer, cross_obs_buffer], training=use_actor_train_call)  # shape (batch, seq_len, 2)
            loss = self.curious_loss_fn(actor_probs, pred_probs)

        curious_grads = tape.gradient(loss, self.curious_model.trainable_variables)
        self.curious_optimizer.apply_gradients(zip(curious_grads, self.curious_model.trainable_variables))
        return loss
