...
import scipy.signal
from task.GA_KnapsackMO4_Task import GA_KnapsackMO4_Task
...
class TrainKnapsackMO4(AbstractTask):
    ...
    def __init__(
        ...
        super(TrainKnapsackMO4, self).__init__(run_num, barrier, problem, epochs, actor_load_path, critic_load_path)
        self.debug = debug
        self.c_type = c_type
        ...
        # HV
        self.ref_point = np.array(config.hv_ref_point_mo4)  # value, weight
        ...
        # Objective Weights
        self.objective_weights = sample_many_obj_weights(4, total=1)
    ...
    def get_cross_obs(self):
        ...
        weight_samples = random.sample(self.objective_weights, num_weight_samples - 4)
        weight_samples.append([0.85, 0.05, 0.05])
        weight_samples.append([0.05, 0.85, 0.05])
        weight_samples.append([0.05, 0.05, 0.85])
        weight_samples.append([0.05, 0.05, 0.05])
        ...
        task_samples_all = []
        for weight, weight_2, weight_3 in weight_samples:
            # sample_vars = [weight, task_cond_vars[1], task_cond_vars[2], task_cond_vars[3]]
            sample_vars = [weight, weight_2, weight_3]
        ...
        return cross_obs_tensor, weight_samples_all, task_samples_all, weight_samples_2_all, weight_samples_3_all
    ...
    def fast_mini_batch(self):
        ...
        cross_obs_tensor, weight_samples_all, task_samples_all, weight_samples_2_all, weight_samples_3_all = self.get_cross_obs()
        str_tracker = {}
    ...
    def calc_reward(self, bitstr, weight, weight_2, weight_3, task, run_val=True):
        ...
        obj_value, obj_value_2, obj_value_3, obj_weight = self.problem.evaluate(bitstr)
        ...
        weight_term = w4 * (1.0 - obj_weight)
        performance_term = value_1_term + value_2_term + value_3_term + weight_term
        ...
        d_objectives = [obj_value * -1.0, obj_value_2 * -1.0, obj_value_3 * -1.0, obj_weight]
...
from problem.knapsack.ManyObjective4KP import ManyObjective4KP

if __name__ == '__main__':
    problem = ManyObjective4KP(
        n=config.num_vars,
        new_problem=False
    )
