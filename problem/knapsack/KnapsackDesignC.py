...
class KnapsackDesignC:
    ...
    def __init__(self, design_vector=None, evaluator=None, num_bits=30, c_type='uniform', p_num=0, val=False, constraints=False):
        ...
        if not design_vector:
            self.vector = KnapsackDesignC.random_design(num_bits)
    ...
    def set_evaluate(self, results):
        value, weight, constraint = results
        ...
        # Constraints
        self.evaluate_constraints(constraint)
    ...
    def evaluate(self):
        if self.evaluated is True:
            ...
            value, weight, constraint = self.evaluator.evaluate(self.vector)
            return self.set_evaluate([value, weight, constraint])
        
    def evaluate_constraints(self, constraint):
        if constraint == 0.0:
            self.is_feasible = True
            self.feasibility_score = 0.0
        else:
            self.is_feasible = False
            self.feasibility_score = constraint
