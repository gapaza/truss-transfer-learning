...
class KnapsackDesignMO6:
    ...
    def __init__(self, design_vector=None, evaluator=None, num_bits=30, c_type='uniform', p_num=0, val=False, constraints=False):
        ...
        if not design_vector:
            self.vector = KnapsackDesignMO6.random_design(num_bits)
    ...
    def set_evaluate(self, results):
        value_1, value_2, value_3, value_4, value_5, weight = results
        value_1 = float(value_1 * -1.0)  # convert to negative value
        value_2 = float(value_2 * -1.0)  # convert to negative value
        value_3 = float(value_3 * -1.0)  # convert to negative value
        value_4 = float(value_4 * -1.0)  # convert to negative value
        value_5 = float(value_5 * -1.0)  # convert to negative value
        weight = float(weight)
        self.objectives = [value_1, value_2, value_3, value_4, value_5, weight]
        self.evaluated = True
    ...
    def evaluate(self):
        if self.evaluated is True:
            ...
            results = self.evaluator.evaluate(self.vector)
            return self.set_evaluate(results)
