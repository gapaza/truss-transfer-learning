...
class ManyObjective4KP(AbstractKnapsack):

    def __init__(self, n=10, new_problem=False, id=''):
        super(ManyObjective4KP, self).__init__(n, new_problem, id)
...
if __name__ == '__main__':
    ...
    kp = ManyObjective4KP(n=n, new_problem=True)

    sol = ''.join([str(1) for _ in range(n)])
    # sol = kp.random_solution()

    value_1, value_2, value_3, weight = kp.evaluate(sol)
